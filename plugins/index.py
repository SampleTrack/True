...
async def index_files(bot, query):
    # Data Format: i#action#chat_id#msg_id#user_id
    # actions: a=accept, r=reject, c=cancel
    ...
    # Get lock for this specific chat
    ...
    await index_files_to_db(int(last_msg_id), chat_id, query.message, bot)
...
async def send_for_index(bot, message):
    ...
    try:
        # Resolve chat to get valid ID/Title
        chat_info = await bot.get_chat(chat_id)
        chat_id = chat_info.id
    except Exception as e:
        return await message.reply(f"❌ Error: {e}")

    # BUTTONS: Extremely short callback data to avoid 64-byte crash
    # i#a = index accept | i#r = index reject
    btns = [[
...
async def index_files_to_db(lst_msg_id, chat, msg, bot):
    ...
    async with get_lock(chat):
        try:
            # Reverse=False means newest to oldest
