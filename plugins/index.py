...
async def cancel_indexing(bot: Client, query):
    temp.CANCEL = True
    await query.answer(
...
async def index_files(bot: Client, query):
    ...
    if int(from_user) not in ADMINS:
        await bot.send_message(
            int(from_user),
            f"Your submission for indexing {chat} has been accepted and will be processed soon.",
            reply_to_message_id=int(lst_msg_id)
        )
    ...
    try:
        ...
        await msg.edit_text(f"Error during indexing:\n{str(e)}")
